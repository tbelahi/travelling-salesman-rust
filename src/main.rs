...
fn main() {
    ...
    let temps = [10f64, 50f64, 100f64, 500f64, 800f64, 1000f64, 2000f64, 3000f64, 4000f64, 5000f64, 6000f64, 10000f64];
    let mut results = vec!();
    temps.par_iter().map(|&i| optimize_travel(&plan, &cities, i, 4f64, 1000)).collect_into(&mut results);
    for i in 0..temps.len() {
        println!("starting temperature: {}", temps[i]);
        println!("Optimized plan: {:?}, cost: {:2}", results[i].0, results[i].1[results[i].1.len() - 1]);
    }
}
